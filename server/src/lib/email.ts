...
const FROM = process.env.RESEND_FROM ?? 'SHAMAR <shamar@send.namite.xyz>'
...
export function reminderEmail(merchant: string, amount: number, currency: string, cadence: string, message?: string): string {
  ...
  <h1 style="color:#E50914;font-size:16px;font-weight:bold;margin:0 0 20px;letter-spacing:0.08em">SHAMAR REMINDER</h1>
  ...
  <a href="https://shamar.namite.xyz/run" style="display:inline-block;margin-top:20px;background:#E50914;color:#fff;padding:10px 20px;text-decoration:none;font-weight:bold;font-size:11px;letter-spacing:0.1em">REVIEW →</a>
  <p style="margin:24px 0 0;color:#3a3a3a;font-size:10px">Shamar — Subscription Intelligence</p>
</div>`
}
